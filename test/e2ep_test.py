...
from End2EndPurification.e2ep import BlockingTimes, prepare_nodes_and_links, LocalBellPairProcessor, calc_new_fidelity_by_entanglement_swapping, calc_new_fidelity_by_purification, calc_success_rate_of_purification
...
class FuncTest(TestCase):
    ...
    def test_calc_new_fidelity_by_entanglement_swapping(self):
        self.assertEqual(calc_new_fidelity_by_entanglement_swapping(0.98, 0.98), 0.98*0.98)
    def test_calc_new_fidelity_by_purification(self):
        self.assertEqual(calc_new_fidelity_by_purification(0.99, 0.98), 0.99*0.98 / (0.99 * 0.98 + 0.01 * 0.02))
    def test_calc_success_rate_of_purification(self):
        self.assertEqual(calc_success_rate_of_purification(0.99, 0.98), 0.98*0.99+0.01*0.02)
...
class LocalBellPairProcessorTest(TestCase):
    def setUp(self) -> None:
        num_node = 5
        self.p_op_int_node = 0.99
        self.p_mem_int_node = 0.99
        self.p_op_end_node = 0.99        
        self.p_mem_end_node = 0.99
        self.fidelity_raw_bellpair = 0.98
        target_fidelity = 0.99
        self.nodes, self.links = prepare_nodes_and_links(num_node, self.fidelity_raw_bellpair, self.p_op_int_node, self.p_mem_int_node, self.p_op_end_node, self.p_mem_end_node)
        self.local_bpps = []
