...
class LocalBellPairProcessorTest(TestCase):
    def setUp(self) -> None:
        num_node = 5
        self.p_intermediate_node = 0.99
        self.p_end_node = 0.99        
        self.fidelity_raw_bellpair = 0.98
        target_fidelity = 0.99
        self.nodes, self.links = prepare_nodes_and_links(num_node, self.fidelity_raw_bellpair, self.p_intermediate_node, self.p_end_node)
        self.local_bpps = []
    ...
    def test_calc_new_blocking_time_by_purification(self):
         for local_bpp in self.local_bpps:
            ...
            if local_bpp.node_left.is_end_node and local_bpp.node_right.is_end_node:
                self.assertEqual(bt.blocking_time_intermediate_node, 0)
                ...
                self.assertEqual(bt.blocking_time_intermediate_node, (2+local_bpp.distance)/success_rate)
                self.assertEqual(bt.blocking_time_end_node, (2+local_bpp.distance)/success_rate)
            else:
                self.assertEqual(bt.blocking_time_intermediate_node, (4+local_bpp.distance*2)/success_rate)
